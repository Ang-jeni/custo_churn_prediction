...
st.markdown("""<style>
    ...
    * { color: #000000 !important; font-family: 'Arial', sans-serif !important; }
    ...
    .risk-high { background: linear-gradient(90deg, #DC2626, #EF4444) !important; color: white !important; padding: 12px 25px; border-radius: 25px;
        font-weight: 900; font-size: 20px; box-shadow: 0 6px 20px rgba(239, 68, 68, 0.3); text-transform: uppercase; letter-spacing: 1px; }
    .risk-medium { background: linear-gradient(90deg, #F59E0B, #FBBF24) !important; color: #000000 !important; padding: 12px 25px; border-radius: 25px;
        font-weight: 900; font-size: 20px; box-shadow: 0 6px 20px rgba(245, 158, 11, 0.3); text-transform: uppercase; letter-spacing: 1px; }
    .risk-low { background: linear-gradient(90deg, #059669, #10B981) !important; color: white !important; padding: 12px 25px; border-radius: 25px;
        font-weight: 900; font-size: 20px; box-shadow: 0 6px 20px rgba(16, 185, 129, 0.3); text-transform: uppercase; letter-spacing: 1px; }
...
# ============================================
# DATA GENERATION (GEO + SENTIMENT + SIM)
# ============================================
...
def build_pipeline(algorithm='Logistic Regression', class_weight='balanced', calibrate=True, random_state=42):
    ...
    if calibrate:
        # safer for small datasets: use cv=2
...
def train_evaluate(df, test_size=0.2, algorithm='Logistic Regression', random_state=42, calibrate=True):
    ...
    pipe = build_pipeline(algorithm=algorithm, class_weight=class_weight_map if algorithm=='Logistic Regression' else 'balanced',
...
def ensure_latlon(df: pd.DataFrame) -> pd.DataFrame:
    """Add latitude and longitude columns based on location if missing."""
